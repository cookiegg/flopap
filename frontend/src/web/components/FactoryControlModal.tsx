...
import { X, RefreshCw, Layers, FileText, Play, Activity, CheckCircle, AlertCircle, Clock, Calendar, BookOpen, Download, Users, Sparkles } from 'lucide-react';
import {
    getFactoryStatus,
    triggerFetchArxiv,
...
const FactoryControlModal: React.FC<FactoryControlModalProps> = ({
    ...
    const [confPoolRatio, setConfPoolRatio] = useState(0.2);  // 推荐池比例 (20%)
    ...
    const handleConfPool = async (confId: string) => {
        try {
            await triggerConferencePool(confId, false, confPoolRatio);
            await fetchConferences();
        } catch (e: any) {
    ...
    const handleConfContent = async (confId: string) => {
        try {
            ...
            await triggerConferenceContent(confId, steps, confContentScope, confPoolRatio);
            await fetchConferences();
        } catch (e: any) {
                        ...
                        <div className={`p-3 rounded-xl border space-y-3 ${isDark ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                            ...
                            <div>
                                <div className="flex justify-between text-xs mb-1">
                                    ...
                                    <span className="text-purple-500 font-mono">{Math.round(confPoolRatio * 100)}%</span>
                                </div>
                                <input
                                    type="range"
                                    min="10"
                                    max="100"
                                    step="5"
                                    value={confPoolRatio * 100}
                                    onChange={(e) => setConfPoolRatio(parseInt(e.target.value) / 100)}
                                    className="w-full h-1.5 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-purple-500"
                                />
                                <div className={`text-[10px] mt-1 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>
                                    {language === 'zh' ? '* 应用于每个会议的推荐池生成' : '* Applied when generating pool for each conference'}
                                </div>
                            </div>
