...
import { X, Flame, Layers, Globe, Moon, Sun, ChevronDown, ChevronRight, Calendar, History, Activity, BookOpen } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // 分离arxiv和会议数据源
  const arxivSource = dataSources.find(s => s.id === 'arxiv');
  ...
  return (
    <>
      ...
      <div className={`fixed md:static top-0 left-0 h-full w-64 flex-shrink-0 ${isDark ? 'bg-black border-gray-800' : 'bg-white border-gray-200'} border-r z-[60] transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 shadow-2xl md:shadow-none flex flex-col`}>
        ...
        {/* Reward QR Code */}
        <div className={`mt-auto pt-6 border-t ${isDark ? 'border-gray-800' : 'border-gray-200'} flex flex-col items-center`}>
          ...
          <div className={`p-2 rounded-xl ${isDark ? 'bg-white/5' : 'bg-white shadow-sm border border-gray-100'}`}>
            <img
