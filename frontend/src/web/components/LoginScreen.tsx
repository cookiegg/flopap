  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform() && googleAuth) {
        ...
        const callbackUrl = `${baseUrl}/v1/auth/callback`;

        const requestBody = {
          provider: 'google',
          code: result.authentication.idToken,
        };
        ...
        if (response.ok) {
          ...
          setPhoneError('Login failed: ' + (data.detail || 'Unknown error'));
        }
      } else {
        // Web端
      ...
      setPhoneError(ui.loginFailed || '登录失败');
    } finally {
      setLoading(false);
    }
  };
