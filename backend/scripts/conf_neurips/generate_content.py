...
def get_papers_without_content(session: Session, limit: int = 10) -> list[Paper]:
    ...
    # 转换为Paper对象并从session中分离，避免多线程session冲突
    papers = []
...
def main():
    ...
    logger.info(f"开始批次 {batch_id} - 为NeurIPS 2025论文生成翻译和AI解读 (deepseek-reasoner)")
    
    with SessionLocal() as session:
        # 获取候选论文
        candidate_papers = get_papers_without_content(session, limit=200)  # 获取更多候选
        ...
        # 限制为100篇进行大批量处理
        papers_need_translation = papers_need_translation[:100]
        papers_need_interpretation = papers_need_interpretation[:100]
    ...
    # 阶段1：生成内容（与数据库分离）
    logger.info("=== 阶段1：增量生成和缓存内容 (deepseek-reasoner) ===")
